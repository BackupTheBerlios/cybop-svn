 ...
 *
 * @version $Revision: 1.19 $ $Date: 2008-07-08 07:11:32 $ $Author: christian $
...
void run_archive(void* p0, void* p1, void* p2, void* p3) {
    ...
    log_terminated_message((void*) INFORMATION_LOG_LEVEL, (void*) L"Run archive command.");
    ...
    if (*createm != *NULL_POINTER) {
        ...
        if (**createm == *NUMBER_1_INTEGER) {
            ...
            if ((argc + *PRIMITIVE_COUNT + *ARCHIVE_UNIX_SHELL_COMMAND_CREATE_COUNT) >= args) {
                ...
                // Determine arguments size.
                args = argc * *POINTER_VECTOR_REALLOCATION_FACTOR + *PRIMITIVE_COUNT + *ARCHIVE_UNIX_SHELL_COMMAND_CREATE_COUNT;
    ...
    if (*updatem != *NULL_POINTER) {
        ...
        if (**updatem == *NUMBER_1_INTEGER) {
            ...
            if ((argc + *PRIMITIVE_COUNT + *ARCHIVE_UNIX_SHELL_COMMAND_UPDATE_COUNT) >= args) {
                ...
                // Determine arguments size.
                args = argc * *POINTER_VECTOR_REALLOCATION_FACTOR + *PRIMITIVE_COUNT + *ARCHIVE_UNIX_SHELL_COMMAND_UPDATE_COUNT;
    ...
    if (*bzip2m != *NULL_POINTER) {
        ...
        if (**bzip2m == *NUMBER_1_INTEGER) {
            ...
            if ((argc + *PRIMITIVE_COUNT + *ARCHIVE_UNIX_SHELL_COMMAND_BZIP2_COUNT) >= args) {
                ...
                // Determine arguments size.
                args = argc * *POINTER_VECTOR_REALLOCATION_FACTOR + *PRIMITIVE_COUNT + *ARCHIVE_UNIX_SHELL_COMMAND_BZIP2_COUNT;
    ...
    if (*createm != *NULL_POINTER) {
        ...
        if (**createm == *NUMBER_1_INTEGER) {
            ...
            if ((commandc + *PRIMITIVE_COUNT + *ARCHIVE_UNIX_SHELL_COMMAND_CREATE_COUNT) >= commands) {
                ...
                // Determine command size.
                commands = commandc * *POINTER_VECTOR_REALLOCATION_FACTOR + *PRIMITIVE_COUNT + *ARCHIVE_UNIX_SHELL_COMMAND_CREATE_COUNT;
    ...
    if (*updatem != *NULL_POINTER) {
        ...
        if (**updatem == *NUMBER_1_INTEGER) {
            ...
            if ((commandc + *PRIMITIVE_COUNT + *ARCHIVE_UNIX_SHELL_COMMAND_UPDATE_COUNT) >= commands) {
                ...
                // Determine command size.
                commands = commandc * *POINTER_VECTOR_REALLOCATION_FACTOR + *PRIMITIVE_COUNT + *ARCHIVE_UNIX_SHELL_COMMAND_UPDATE_COUNT;
    ...
    if (*bzip2m != *NULL_POINTER) {
        ...
        if (**bzip2m == *NUMBER_1_INTEGER) {
            ...
            if ((commandc + *PRIMITIVE_COUNT + *ARCHIVE_UNIX_SHELL_COMMAND_BZIP2_COUNT) >= commands) {
                ...
                // Determine command size.
                commands = commandc * *POINTER_VECTOR_REALLOCATION_FACTOR + *PRIMITIVE_COUNT + *ARCHIVE_UNIX_SHELL_COMMAND_BZIP2_COUNT;
