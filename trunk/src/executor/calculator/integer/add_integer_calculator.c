...
void calculate_integer_add(void* p0, void* p1) {
    ...
    if (p1 != *NULL_POINTER_MEMORY_MODEL) {
        ...
        if (p0 != *NULL_POINTER_MEMORY_MODEL) {
            ...
            log_terminated_message((void*) DEBUG_LEVEL_LOG_MODEL, (void*) L"Calculate integer add.");

            *sum = *sum + *s;

        } else {

            log_terminated_message((void*) ERROR_LEVEL_LOG_MODEL, (void*) L"Could not calculate integer add. The sum is null.");
        }

    } else {

        log_terminated_message((void*) ERROR_LEVEL_LOG_MODEL, (void*) L"Could not calculate integer add. The summand is null.");
    }
}
